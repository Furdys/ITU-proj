...
class Board(QWidget):
    ...
    def AnimFinish(self):
        ...
        if (self.Matrix[self.selectedPosition[1]][self.selectedPosition[0]].hasPawn == True):
            # print("IF" , self.Matrix[self.selectedPosition[1]][self.selectedPosition[0]].posX, self.Matrix[self.selectedPosition[1]][self.selectedPosition[0]].posY)
            j = 0
        ...
        for i in range(len(self.pawns)):
            if self.pawns[i].position[0] == self.oldPosition[0] and self.pawns[i].position[1] == self.oldPosition[1]:
                # print("Pred", self.pawns[i].position[0],self.pawns[i].position[1])
                self.pawns[i].position[0] = self.selectedPosition[0]
                self.pawns[i].position[1] = self.selectedPosition[1]
                # print("Po", self.pawns[i].position[0],self.pawns[i].position[1])
                break

        self.playerPlaying = abs(self.playerPlaying - 1)
        self.playerChanged.emit()
        self.passClickToField.emit(-1,-1)
    ...
    def fieldWasClickedBoard(self, posY, posX, player): # pass signal to field
        ...
        if (self.pawnSelected == False and self.playerPlaying != player):
            ...
            #Find it
            for l in range(len(self.pawns)):
                if self.pawns[l].position[0] == self.selectedPosition[0] and self.pawns[l].position[1] == self.selectedPosition[1]:
                    paw = self.pawns[l]

            moves = [[0 for col in range(8)] for row in range(8)]
            for i in range(8):
                for j in range(8):
                    if (self.Matrix[i][j].hasPawn):
                        moves[i][j] = 1
                    else:
                        moves[i][j] = 0
            print (moves)

            ts.findLegalMoves(moves, self.selectedPosition[1],self.selectedPosition[0], paw.type.lower())
